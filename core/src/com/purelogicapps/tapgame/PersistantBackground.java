...
public class PersistantBackground {
	...
	public static void setBackground(Texture texture, float scrollSpeedHoriz, float scrollSpeedVert){
		if(scrollSpeedHoriz <= 0 || scrollSpeedVert <= 0){
			throw new IllegalArgumentException("Cannot have 0 or negative scroll speed.");
		}
		if(texture == null){
			throw new IllegalArgumentException("Background texture must not be null.");
		}
		PersistantBackground.batch.disableBlending();
	 ...
	 * <b>Repeat:</b> If true the texture will be repeated in that direction if it cannot fill the screen
	 * otherwise it will be stretched.<br />
	 * <b>Reverse:</b> if true the scrolling will be reversed in that direction.
	 */
